import type { BackendHealth, BatchReport, BatchSettings, CrmCompany, CrmCompanyDetail, CrmContact, DashboardData, IntegrationSettings, JobRow, MailboxEvent, MessageDraft, OutreachHistoryItem, PreflightResult, QueueItem, RecoveryException, SetupStatus, WorkspaceSnapshot } from "../types";
...
export function generateMessages(backendUrl: string, input: { brief: string; drafts: MessageDraft[]; maximum_words: number }): Promise<Array<{ draft_id: string; subject: string; body: string }>> {
